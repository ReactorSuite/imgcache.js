    	...
    	if(file_entry === null){
    		...
    		file_entry.getMetadata(function (metadata) {
    			if(metadata["size"]){
        			...
        			if(changed){
        		        ImgCache.overridables.log("Removing updated file: " + img_src, LOG_LEVEL_INFO);
        				ImgCache.removeFile(img_src, function(){
        					ImgCache.overridables.log("Successfull removed updated file: " + img_src, LOG_LEVEL_INFO);
        					response_callback(src, changed);
        				}, function(){
        					ImgCache.overridables.log("Failed to removed updated file: " + img_src, LOG_LEVEL_INFO);
        					response_callback(src, changed);
        				});
        			}
    			}
