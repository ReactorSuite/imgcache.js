...
(function ($) {
  ...
  Private.FileTransferWrapper.prototype.abort = function(){
      return new Promise((resolve, reject) => {
		  this.fileTransfer.abort().then(() => {
			  resolve();
		  }).catch(error => {
			  reject(error);
		  });
  ...
  ImgCache.cancelDownloads = function(){
	  ImgCache.overridables.log("ImgCache.cancelDownloads", LOG_LEVEL_INFO);
	  var abortPromises = [];
	  if(ImgCache.currentDownloads && ImgCache.currentDownloads.length > 0){
		  ImgCache.overridables.log("ImgCache.cancelDownloads - there are " + ImgCache.currentDownloads.length + " to cancel.", LOG_LEVEL_INFO);
		  for(var i = 0; i < ImgCache.currentDownloads.length; i++){
			  ImgCache.overridables.log("ImgCache.cancelDownloads - " + i, LOG_LEVEL_INFO);
			  abortPromises.push(ImgCache.currentDownloads[i].fileTransfer.abort());
		  }
	  }
	  return new Promise((resolve,reject) => {
		  Promise.all(abortPromises).then(() => {
			  ImgCache.overridables.log("ImgCache.cancelDownloads cancelled " + abortPromises.length + " downloads.", LOG_LEVEL_INFO);
			  resolve();
		  }).catch(error=>{
			  ImgCache.overridables.log("ImgCache.cancelDownloads failed " + error, LOG_LEVEL_INFO);
			  reject(error);
		  });
	  });
  }
  ...
  ImgCache.cacheFile = function (img_src, success_callback, error_callback, on_progress) {
    ...
    ImgCache.currentDownloads.push(fileTransfer);
    fileTransfer.download(
      img_src,
      filePath,
      function (entry) {
    	ImgCache.currentDownloads.splice(ImgCache.currentDownloads.indexOf(fileTransfer), 1);  
        ...
        if (entry.setMetadata) {
        	ImgCache.overridables.log('setting metadata', LOG_LEVEL_INFO);
        	var meta = {
        		'com.apple.MobileBackup': 1
        	};
        	ImgCache.overridables.log(meta, LOG_LEVEL_INFO);
          entry.setMetadata(
            function () {
              /* success*/
              ImgCache.overridables.log('com.apple.MobileBackup metadata set', LOG_LEVEL_INFO);
  ...
  // Reminder: this is an asynchronous method!
  // Answer to the question comes in response_callback as the second argument (first being the path)
  ImgCache.isCached = function (size, img_src, response_callback) {
    ImgCache.getCachedFile(img_src, function (src, file_entry) {
    	if(file_entry === null){
            ImgCache.overridables.log(img_src + ' was not in cache', LOG_LEVEL_INFO);
    		response_callback(src, false);
    	}
    	else{
    		file_entry.getMetadata(function (metadata) {
    			ImgCache.overridables.log(metadata, LOG_LEVEL_INFO);
    			if(metadata["size"]){
    				//check to see if the file on disk is the same size as the file being checked.
    				var changed = metadata["size"] != size;
        			ImgCache.overridables.log(img_src + ' has ' + (changed ? 'changed' : 'not changed'), LOG_LEVEL_INFO);
        			//if the file has changed, remove the old one
        			if(changed){
        		        var filePath = Private.getCachedFileFullPath(img_src);
        		        ImgCache.overridables.log("Removing updated file: " + filePath, LOG_LEVEL_INFO);
        				ImgCache.removeFile(img_src, function(){
        					ImgCache.overridables.log("Successfull removed updated file: " + filePath, LOG_LEVEL_INFO);
        					//tell the calling method this file is not cached so it gets reloaded
        					response_callback(src, false);
        				}, function(error){
        					ImgCache.overridables.log(error, LOG_LEVEL_ERROR);
        					ImgCache.overridables.log("Failed to removed updated file: " + filePath, LOG_LEVEL_INFO);
        					//tell the calling method this file is not cached so it gets reloaded
        					response_callback(src, false);
        				});
        			}
        			//if the file has not changed, return true so it does not get reloaded
        			else{
        				response_callback(src, true);
        			}
    			}
    			else{
    				//since the metadata didn't have a size, return false so the file gets loaded.
    				response_callback(src, false);
    			}
        	});
    	}
