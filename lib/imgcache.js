...
(function ($) {
  ...
  ImgCache.cacheFile = function (lastModified, img_src, success_callback, error_callback, on_progress) {
  ...
  // Reminder: this is an asynchronous method!
  // Answer to the question comes in response_callback as the second argument (first being the path)
  ImgCache.isCached = function (lastChanged, img_src, response_callback) {
    ImgCache.getCachedFile(lastChanged, img_src, function (src, file_entry) {
    	if(file_entry === null){
    		...
    		file_entry.file(function(file) {
    			var changed = file.lastModifiedDate == lastChanged;
    			ImgCache.overridables.log(img_src + ' has ' + (changed ? 'changed' : 'not changed'), LOG_LEVEL_INFO);
    			response_callback(src, file.lastModifiedDate == lastChanged);
        	});
    	}
    });
  };
  ...
  ImgCache.cacheBackground = function ($div, success_callback, error_callback, on_progress) {
    ...
    ImgCache.cacheFile(undefined, img_src, success_callback, error_callback, on_progress);
  };
